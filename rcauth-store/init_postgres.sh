...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
    ...
    create user redcardinal_auth_admin noinherit createrole login noreplication password '$POSTGRES_PASSWORD';
    create schema if not exists $DB_NAMESPACE authorization redcardinal_auth_admin;
    grant create on database $POSTGRES_DB to redcardinal_auth_admin;
